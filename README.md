...
- `/agent`: 统一 AI 工作台，把多轮对话、附件分析和任务执行放在同一界面；已接入动作会自动执行并生成可追踪记录，外部平台或未接工具的动作会保存为待接入步骤。旧 `/assistant` 地址会自动跳转到这里。
...
AI 工作台通过服务端接口调用模型或 Hermes Agent，前端不会接触 API Key。对话、消息、附件提取文本和执行轨迹会保存到 SQLite，方便按对话继续上下文。`AI_BACKEND=hermes` 时，每个 Nekko 对话会带独立 Hermes session header，Hermes 网关离线时可自动降级到 DeepSeek 直连。

Workspace Agent 会先自主规划完整执行路径。创建项目、创建任务、创建灵感、创建资料、运行 B站内容雷达、智能整理灵感库等已接入动作会由服务端校验后自动执行；外部平台登录、发布、通知、非公开采集、删除和权限变更等还没接入工具或需要账号授权的动作，会保留为“待接入/待授权”步骤，而不是直接判失败或假装完成。
