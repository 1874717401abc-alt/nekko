...
- `/agent`: 任务执行中心，把一句目标拆成可追踪步骤；已接入动作自动执行，外部平台或未接工具的动作会保存为待接入步骤。
...
Workspace Agent 会先自主规划完整执行路径。创建项目、创建任务、创建灵感、创建资料、运行 B站内容雷达、整理内容雷达标签等已接入动作会由服务端校验后自动执行；外部平台登录、发布、通知、非公开采集、删除和权限变更等还没接入工具或需要账号授权的动作，会保留为“待接入/待授权”步骤，而不是直接判失败或假装完成。
