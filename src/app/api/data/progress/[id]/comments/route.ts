...
import { createProgressComment } from "@/lib/resourceRules";
...
export async function POST(
  ...
  const result = createProgressComment(body as Record<string, unknown>, user);
  if ("error" in result) {
  ...
  const updated = updateDataItem<ProgressTask>("progress", id, (task) => ({
    ...task,
    comments: [...(task.comments ?? []), result.item],
  }));
