...
export default function ProjectScriptReview({ projectId, scenes, initialVersions, initialReviews }: { projectId: string; scenes: ScriptScene[]; initialVersions: ScriptVersion[]; initialReviews: ScriptReview[] }) {
  ...
  const activeSceneId = scenes.some((item) => item.id === sceneId) ? sceneId : scenes[0]?.id ?? "";
  const scene = scenes.find((item) => item.id === activeSceneId);
  const sceneVersions = useMemo(() => versions.filter((item) => item.sceneId === activeSceneId).sort((a, b) => b.version - a.version), [versions, activeSceneId]);
  const sceneReviews = reviews.filter((item) => item.sceneId === activeSceneId);
  ...
  async function addReview(event: React.FormEvent) { event.preventDefault(); if (!activeSceneId) return; setPending(true); try { const created = await createItem<ScriptReview>("scriptReviews", { projectId, sceneId: activeSceneId, versionId: sceneVersions[0]?.id, content: comment }); setReviews((current) => [created, ...current]); setComment(""); } catch (err) { setError(err instanceof Error ? err.message : "审阅意见保存失败。"); } finally { setPending(false); } }
  ...
  return <section className="mt-10 border-t border-line pt-7" aria-labelledby="review-heading">
    <div className="mb-5 flex flex-wrap items-start justify-between gap-3"><div><h2 id="review-heading" className="text-lg font-semibold text-ink">版本与审阅</h2><p className="mt-1 text-xs text-ink-soft">保存不可变快照，再进入审核、通过和锁定。</p></div><select value={activeSceneId} onChange={(e) => setSceneId(e.target.value)} className="h-9 max-w-full rounded-md border border-line bg-paper px-3 text-xs focus:border-accent focus:outline-none">{scenes.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select></div>
