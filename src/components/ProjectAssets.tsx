...
import { ArrowUpRight, FileText, Image as ImageIcon, Trash2, Upload, Video } from "lucide-react";
import { deleteItem } from "@/lib/clientData";
...
export default function ProjectAssets({ projectId, initialAssets, inspiration, library }: { projectId: string; initialAssets: ProjectAsset[]; inspiration: InspirationItem[]; library: LibraryItem[] }) {
  ...
  async function upload(event: React.FormEvent) {
    ...
    const form = new FormData(); form.set("file", file); form.set("projectId", projectId); form.set("kind", kind); form.set("tags", tags); form.set("note", note);
    try { const response = await fetch("/api/project-assets", { method: "POST", body: form }); const data = await response.json(); if (!response.ok) throw new Error(data.error || "上传失败。"); setAssets((current) => [data as ProjectAsset, ...current]); setTags(""); setNote(""); if (fileRef.current) fileRef.current.value = ""; }
  ...
  return <div>
    <section aria-labelledby="asset-file-heading">
      ...
      <form onSubmit={upload} className="mb-7 grid gap-3 border-y border-line/70 py-5 sm:grid-cols-2 lg:grid-cols-[minmax(220px,1fr)_140px_minmax(160px,0.7fr)_minmax(180px,1fr)_auto]">
        ...
        <button disabled={pending} className="inline-flex h-10 items-center justify-center gap-2 rounded-md bg-accent px-4 text-xs font-medium text-white disabled:opacity-50"><Upload className="h-4 w-4" />{pending ? "上传中…" : "上传"}</button>
      </form>
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{assets.map((item) => <article key={item.id} className="overflow-hidden rounded-md border border-line bg-card"><a href={item.url} target="_blank" rel="noreferrer" className="block aspect-video overflow-hidden bg-paper-soft">{item.kind === "image" ? <>{/* Authenticated uploads intentionally bypass the public image optimizer. */}<img src={item.url} alt={item.title} className="h-full w-full object-cover" /></> : item.kind === "video" ? <video src={item.url} controls preload="metadata" className="h-full w-full bg-black object-contain" /> : <span className="flex h-full items-center justify-center"><FileText className="h-8 w-8 text-ink-soft" /></span>}</a><div className="p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-medium text-ink">{item.title}</p><p className="mt-1 text-[11px] text-ink-soft">{kindLabel[item.kind]} · V{item.version} · {sizeLabel(item.size)}</p></div><button type="button" onClick={() => remove(item)} title="删除" aria-label="删除素材" className="flex h-7 w-7 shrink-0 items-center justify-center rounded text-ink-soft hover:bg-red-500/10 hover:text-red-500"><Trash2 className="h-3.5 w-3.5" /></button></div>{item.note && <p className="mt-2 line-clamp-2 text-xs leading-5 text-ink-soft">{item.note}</p>}{item.extractedText && <p className="mt-2 inline-flex items-center gap-1 text-[10px] text-emerald-600"><FileText className="h-3 w-3" />Agent 可读取文字</p>}</div></article>)}{assets.length === 0 && <div className="border-y border-dashed border-line py-12 text-center sm:col-span-2 xl:col-span-3"><Upload className="mx-auto h-5 w-5 text-ink-soft" /><p className="mt-3 text-xs text-ink-soft">还没有上传项目文件。</p></div>}</div>
    </section>
